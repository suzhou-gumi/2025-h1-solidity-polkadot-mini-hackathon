...
async function main() {
    ...
    try {
        ...
        // 3. 用户认购（只认购500个，低于软顶1000）
        console.log("\n2. 用户认购（低于软顶）...");
        const amount = ethers.parseEther("500"); // 认购500个资金代币
        ...
        // 8. 用户领取退款
        console.log("\n7. 用户领取退款...");
        const beforeBalance = await fundingToken.balanceOf(deployer.address);
        console.log("领取前资金代币余额:", ethers.formatEther(beforeBalance));

        const claimTx = await launchpad.connect(deployer).claimOrRefund(projectId);
        console.log("领取退款交易已发送:", claimTx.hash);
        await claimTx.wait();

        const afterBalance = await fundingToken.balanceOf(deployer.address);
        console.log("领取后资金代币余额:", ethers.formatEther(afterBalance));
        console.log("退款数量:", ethers.formatEther(afterBalance - beforeBalance));
